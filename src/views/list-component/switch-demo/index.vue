<template>
  <div class="switch-demo">
    <a-card title="Switch 开关组件演示" class="demo-card">
      <!-- 配置区域 -->
      <div class="config-area">
        <a-space>
          <a-checkbox v-model:checked="switchConfig.loading">显示加载状态</a-checkbox>
          <a-checkbox v-model:checked="switchConfig.disabled">禁用所有开关</a-checkbox>
          <a-checkbox v-model:checked="switchConfig.small">小尺寸开关</a-checkbox>
          <a-select v-model:value="switchConfig.size" style="width: 100px">
            <a-select-option value="default">默认尺寸</a-select-option>
            <a-select-option value="small">小尺寸</a-select-option>
          </a-select>
        </a-space>
      </div>
      <!-- 返回按钮 -->
      ...
      <div class="batch-actions" v-if="selectedRowKeys.length > 0">
        <a-space>
          ...
          <a-button @click="batchToggleNotifications">切换通知</a-button>
        ...
        >
          ...
          <template #bodyCell="{ column, record, index }">
            <div class="ant-table-column">
              <!-- 状态列自定义渲染 -->
              <template v-if="column.slotName === 'statusSlot'">
              ...
              <a-switch
                ...
                :disabled="isDisabled(column, record) || switchConfig.disabled"
                :loading="switchConfig.loading"
      ...
      <div class="switch-stats">
        ...
        <div class="stats-grid">
          <div class="stat-item">
            ...
            <div class="stat-label">已启用用户</div>
          </div>
          <div class="stat-item">
          ...
          <div class="stat-item">
            <div class="stat-value">{{ notificationsEnabledCount }}</div>
            <div class="stat-label">通知开启用户</div>
          </div>
          <div class="stat-item">
...
import type { AntTableConfig } from '../component/type/index';
...
const enabledCount = computed(() => {
  return tableData.value.filter(user => user.enabled).length;
});
...
const notificationsEnabledCount = computed(() => {
  return tableData.value.filter(user => user.notifications).length;
});
...
const handleSelectionChange = (keys: any[], rows: any[]) => {
  ...
  message.info(`选中了 ${rows.length} 条数据`);
};
...
const handleCellEvent = (type: string, value: any, record: any, dataIndex: string) => {
  ...
  switch (type) {
    ...
    case 'linkClick':
      ...
      break;
    case 'switchChange':
      handleSwitchChange(value, record, dataIndex);
      break;
  }
};

const handleSwitchChange = (checked: boolean, record: UserData, dataIndex: string) => {
  // 模拟API调用
  tableConfig.loading = true;
  ...
  setTimeout(() => {
    ...
    if (user) {
      (user as any)[dataIndex] = checked;
      message.success(`${record.name} 的 ${getSwitchLabel(dataIndex)} ${checked ? '已启用' : '已禁用'}`);
    }
    tableConfig.loading = false;
  }, 300);
};

const getSwitchLabel = (dataIndex: string): string => {
  const labelMap: Record<string, string> = {
    enabled: '启用状态',
    notifications: '通知功能'
  };
  return labelMap[dataIndex] || dataIndex;
};
...
const batchEnable = () => {
  ...
  tableConfig.loading = true;
  setTimeout(() => {
    selectedRows.value.forEach(user => {
      const target = mockData.find(item => item.id === user.id);
      if (target) {
        target.enabled = true;
      }
    });
    message.success(`已批量启用 ${selectedRows.value.length} 个用户`);
    clearSelection();
    tableConfig.loading = false;
  }, 500);
};
...
const batchDisable = () => {
  ...
  setTimeout(() => {
    ...
    message.success(`已批量禁用 ${selectedRows.value.length} 个用户`);
    clearSelection();
    tableConfig.loading = false;
  }, 500);
};

const batchToggleNotifications = () => {
  if (selectedRows.value.length === 0) {
    message.warning('请先选择要操作的数据');
    return;
  }
  ...
  setTimeout(() => {
    ...
    message.success(`已批量切换 ${selectedRows.value.length} 个用户的通知设置`);
    clearSelection();
    tableConfig.loading = false;
  }, 500);
};
